...
void main() => runApp(const MaterialApp(home: Scaffold(body: Center(child: Text('Frigo Recettes Ready')))));
...
class ApiService {
  ...
  static Future<String> genererRecettes() async {
    try {
      ...
      final response = await http.post(url, headers: {"Content-Type": "application/json"}, body: '{"contents": [{"parts": [{"text": "Test"}]}]}');
